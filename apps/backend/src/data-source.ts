...
import { assertDatabaseEnv, resolveEnvFile } from './config/env-file';
...
// A *present* .env.production under NODE_ENV=production is an error, though: resolveEnvFile refuses
// it, which is what keeps `start:prod:migrate` on a laptop from migrating the live database.
...
// Render is unaffected: it runs the compiled file, and its variables come from the platform. The
// compiled file on a laptop is resolveEnvFile's job below; this guard stays because its message is the
// right one for the CLI whether or not the file exists.
...
loadEnv({ path: resolveEnvFile(), quiet: true });
