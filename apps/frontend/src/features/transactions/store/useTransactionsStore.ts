...
export const useTransactionsStore = create<TransactionsState>()(
  persist(
    (set, get) => ({
      ...
      addExpense: (expense) =>
        set((state) => ({
          transactions: [...state.transactions, { ...expense, id: crypto.randomUUID() }],
        })),

      updateTransaction: (id, data) =>
        set((state) => ({
          transactions: state.transactions.map((e) => (e.id === id ? { ...e, ...data } : e)),
        })),
      ...
      // QUERY OPERATIONS
      // ============================================================
      getTransactionsByEvent: (eventId) => get().transactions.filter((e) => e.eventId === eventId),

      getTransactionsByEventPaginated: (eventId, numberOfDates = 10, offset = 0) => {
        const allTransactions = get()
          .transactions.filter((t) => t.eventId === eventId)
        ...
        const paginatedTransactions = allTransactions.filter((t) => datesToLoad.includes(t.date));
      ...
      getTotalExpensesByEvent: (eventId) =>
        get()
          .transactions.filter((e) => e.eventId === eventId && e.paymentType === 'expense')
          .reduce((sum, e) => sum + e.amount, 0),

      getTotalContributionsByEvent: (eventId) =>
        get()
          .transactions.filter((e) => e.eventId === eventId && e.paymentType === 'contribution')
          .reduce((sum, e) => sum + e.amount, 0),

      getTotalCompensationsByEvent: (eventId) =>
        get()
          .transactions.filter((e) => e.eventId === eventId && e.paymentType === 'compensation')
          .reduce((sum, e) => sum + e.amount, 0),
